...
| T13 | **Shared-session replay, stale publication, or transfer takeover** — a lost response duplicates a provider/effect call, a stale device publishes after lease takeover, recovery resumes under changed provider authority, or a pairing code is claimed twice. | #628 binds UUID requests to route/session/semantic payload, journals every provider step before transport, treats ambiguous calls as outcome-unknown, and publishes staged encrypted records only through a renewed fenced manifest CAS. Recovery revalidates the complete provider-generation/harness/installation binding. Pairing V2 is confirmation-gated, five-minute, one-time, and permanently bound to the first durable claim. Host-owned cancellation and terminal ordering prevent late results from becoming durable (REQ-AGENT-016). |
