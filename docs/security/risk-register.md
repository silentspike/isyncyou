...
| **Status** | **Implementation complete / partial evidence** — host, UI, Android contract, hook-isolation, and real Claude/Codex Disconnect/Reconnect evidence are recorded in [the #645 manifest](../evidence/issue-645-manifest.json). The mandatory physical two-account Codex Switch was not repeated after the owner ended further interactive sign-ins, so AC-3 is explicitly recorded as open and REQ-AGENT-015 remains planned. Residual provider revoke-scope and identity-contract drift remains monitored. Design: [ADR-007](../adr/007-agent-architecture.md). |
