...
| **Mitigation** | (a) Cloud-mutating restore is **off by default** (`restore.cloud_restore_enabled = false`), enforced at the engine entry point before any store or network access. (b) The crash-safe design — operation ledger, explicit state machine, content-derived `HMAC-SHA256` idempotency key, reconcile-on-recovery, crash-injection matrix — is implemented and, for **mail**, now **wired into the live `restore_cloud` path with recovery on daemon boot**; it is proven against a non-idempotent fake cloud. The other services still use the direct path (migration tracked in `docs/requirements/restore.yml`). (c) Local-file restore is unaffected and always available. |
| **Status** | **In progress** — gate mitigated + tested; ledger wired for mail + daemon recovery, proven in isolation. The one remaining step before the gate can be turned on is a **live confirmation** (that Graph returns a posted `Message-ID` via an `internetMessageId` `$filter`), which needs a write token. Design: [ADR-001](../adr/001-restore-semantics.md); overview: `SHOWCASE.md` §1. |
