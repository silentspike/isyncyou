...
| **Status** | **Implementation complete / evidence pending** — host, UI, and Android contract tests are present. REQ-AGENT-015 remains planned until the #645 default-device, real-provider lifecycle, hook-isolation, and evidence-manifest gates complete. Residual provider revoke-scope and identity-contract drift remains monitored. Design: [ADR-007](../adr/007-agent-architecture.md). |
