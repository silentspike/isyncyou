...
| Standalone Android app | ✅ | the same UI from an **on-device embedded engine** (no daemon/server), bottom-tab navigation, responsive at 390 px (CI smoke asserts no horizontal overflow across all views). Ships as a **signed release APK** with per-build `versionName`/`versionCode`, built once and attached to every GitHub Release (cosign-signed, checksum in `SHA256SUMS`); CI CodeQL-scans it and an **emulator smoke** boots it and asserts the WebView loads; OTA updates via **Obtainium** ([docs/android-distribution.md](docs/android-distribution.md)). Live-verified on a physical Pixel 8 Pro, including the **FCM push end-to-end** (daemon → device notification). *(Automating that push proof in CI is a 1.1 follow-up — it needs the Firebase service-account as a CI secret.)* |
